...
### 6-A.5 PC 전용 고정 레이아웃 (R8.6)

**반응형을 하지 않는다.** 폭에 따라 셀을 재배치하면 열 너비와 체크박스 줄바꿈이 원본과 달라지고, "종이 서식과 같은 구조"라는 전제가 깨진다. 좁은 화면에서는 `overflow-x: auto` 로 가로 스크롤만 허용해 레이아웃을 그대로 유지한다.

- 서식 폭은 `940px` 고정.
- 표는 `<table table-layout: fixed>` + `<colgroup>`. 행마다 분할이 다르므로 각 셀이 `colSpan`/`rowSpan`으로 묶는다.
- 열 너비는 **원본 PDF의 실제 선 좌표**에서 뽑는다 (R8.9). 서식1 기준 표 폭 340.2pt에 대한 비율:
...
| 10.7 / 83.1 | 저축목적 행 (라벨 · 내용 · "본인이 선택체크") |

  좌표 추출은 PyMuPDF `page.get_drawings()` 로 수직선 x를 모아 정규화한다.

**체크박스 줄바꿈도 고정한다** (R8.6). 원본의 줄당 개수를 그대로 쓴다.
...
**날짜는 년·월·일 3분할** (R8.8). `<input type="date">` 는 칸 안에 브라우저 위젯을 그려 서식 모양을 깬다. 숫자 입력 3개로 받고 내부에서 `YYYY-MM-DD` 로 합친다.

> ⚠️ **원본 PDF가 2-up이다.** 시행지침·공고문 모두 A4 가로(841×595pt) 한 장에 논리 2쪽이 들어 있다. 좌표를 뽑을 때 반드시 반쪽으로 잘라야 한다.
